...
- "approval" overloaded between Diff Block review and shell command review — resolved (2026-06-21): **Approval** in awecode means **Diff Block** review only. `ApprovalQueue` accepts `ParsedDiff`, not shell commands. `shell_exec` tool runs without approval (only 60s timeout). Future "shell approval" would be a distinct feature requiring `@awecode/agent` extension.
- "web client" vs "desktop client" vs "TUI client" — resolved (2026-06-21): **Client** = any frontend (TUI / Desktop / Web). All three are thin shells over the same `runChatLoop` from `@awecode/agent`. Wire protocol `GuiAgentEvent` / `GuiClientCommand` is shared between Desktop and Web; TUI uses direct callback injection.
- "session resume" — resolved (2026-06-21): sidebar lists past sessions (read-only transcript replay). Continuing to chat in a resumed session is **Session Resume by Replay** (out of scope for v0.1 web and desktop). Distinct from simply viewing history.
- "persistence layer" — resolved (2026-06-21): `packages/agent/src/persistence/` owns anything written to disk for cross-session recall: `sessions.ts` (conversation history metadata + messages), `session-event-handler.ts` (pure transformation `GuiAgentEvent` → `Session` mutation, shared by Desktop AgentBridge and Web ws-bridge), and `checkpoint.ts` (context-window snapshots). Distinct from `context/` (in-memory `ContextManager` + entries + trigger logic) and `chat.ts` (loop control).
- "wire protocol" vs "shared types" — resolved (2026-06-21): `packages/gui/src/shared/protocol.ts` (re-exported via `@awecode/gui/shared/protocol`) owns ONLY wire-format types (`GuiAgentEvent`, `GuiClientCommand`, `SessionMeta`, `Session`, `ContextEntrySnapshot`). Desktop-only presentation types like `WorkspaceState` live in `packages/gui/src/main/types.ts` and are NOT shared with Web.
- "Protocol Session" — resolved (2026-06-21): a `ProtocolSession` object (in `@awecode/agent`) encapsulates one agent conversation as a transport-agnostic event sink. It owns `liveMessages`, `abortController`, lazy `Orchestrator`, and exposes `handlePrompt(text)` / `abort()` / `dispose()`. The Desktop CLI `--internal` mode wraps a ProtocolSession with a stdio transport; the Web `ws-bridge` wraps one with a WebSocket transport. TUI (`chat.tsx`) bypasses it and calls `runChatLoop` directly because Ink is a UI runtime, not a wire transport.
- "transport adapter" — resolved (2026-06-21): the layer that converts a `ProtocolSession`'s event emission into bytes on a specific transport (stdio NDJSON, WebSocket JSON frames). Each transport is ~30 lines of glue: `send(ev)` → `process.stdout.write(JSON.stringify(ev) + '\n')` or `ws.send(JSON.stringify(ev))`. Transports do NOT contain agent logic.
